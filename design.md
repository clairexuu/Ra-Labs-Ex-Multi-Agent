...
This project implements the **Investment Team** track of the Agno multi-agent take-home exercise. The system uses 4 AI agents orchestrated via an Agno Team in **coordinate mode** to research companies, analyze financials, assess risks, and produce a final investment recommendation memo. The demo runs through the **Agno Playground UI**.
...
```
              ...
              └─────────────┬───────────────┘
               ...
               Step 1: Delegate to Research
                   ...
                   │ DuckDuckGo + │  Gathers financial data + news
        ...
        └───────┬───────┘   └──────┬───────┘
                └────────┬─────────┘
                         │
         Step 3: Coordinator collects all outputs
...
| Specialist #1 | **Research Agent** | Comprehensive data gathering using DuckDuckGo (web search) and YFinance (financial data). Collects general info, financials, news, and negative news for all downstream agents |
...
1. **Research** — Coordinator delegates to Research Agent, which uses DuckDuckGo + YFinance tools to gather comprehensive data on each company (general info, financials, news, negative news/lawsuits/regulatory issues)
...
4. **Final Memo** — Coordinator synthesizes all specialist outputs into a markdown investment memo with sections: Executive Summary, Company Overviews, Comparative Analysis, Risk Assessment, Recommendation (BUY/HOLD/SELL), Open Questions

### Why Agno Team Coordinate Mode

- The Team coordinator acts as both the **coordinating agent** and the **output/review step**
...
|------|--------|---------|---------|
| **DuckDuckGoTools** | `app/tools/search.py` | Research Agent | Web search for company news, products, competitive position, lawsuits, regulatory issues |
...
If YFinance or DuckDuckGo tools fail for a company (invalid ticker, API timeout, rate limit), the Research Agent's instructions say: *"If a tool call fails or returns no data for a company, note the data gap and continue with available information."* The Critic Agent will then flag these gaps in its risk assessment.
...
| `duckduckgo-search` / `ddgs` | Web search for news, lawsuits, regulatory info |
...
4. Open **http://localhost:3000** in your browser. It connects to the AgentOS backend at `localhost:7777` by default.
...
Expected: 9 tests pass (5 schema tests + 4 agent/team creation tests).
...
| Tools | DuckDuckGo + YFinance only | Exercise says 2-4 tools; these cover web search + financial data. Research Agent gathers all data for downstream agents |
...
| Scope | 4 agents, 2 tools, typed state, Playground demo | Per the "do not overbuild" guidance in the exercise |
